...
typedef int (*trie_visitor)(const char *key,unsigned int sec, void *data, void *arg);
...
unsigned int  trie_search_sec(const struct trie *, const char *key);
 ...
 * @return 0 on success
 */
int trie_insert(struct trie *, const char *key, void *data,unsigned int sec);
 ...
 * @return 0 on success
 */
int trie_replace(struct trie *, const char *key, trie_replacer f, void *arg,unsigned int sec);
...
int trie_visit_count(struct trie *, const char *prefix, trie_visitor v, void *arg);
